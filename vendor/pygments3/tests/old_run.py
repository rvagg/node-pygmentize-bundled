...
def err(file, what, exc):
    print(file, 'failed %s:' % what, end=' ', file=sys.stderr)
...
def run_tests(with_coverage=False):
    ...
    print(('Pygments %s Test Suite running%s, stand by...' %
         (pygments.__version__,
          with_coverage and " with coverage analysis" or "")).center(WIDTH), file=sys.stderr)
    print(('(using Python %s)' % sys.version.split()[0]).center(WIDTH), file=sys.stderr)
    ...
    for testfile in files:
        ...
        try:
            exec(compile(open(join(testdir, testfile)).read(), join(testdir, testfile), 'exec'), globs)
    ...
    if failed:
        print('%d of %d tests failed.' % \
              (error_test_count, total_test_count), file=sys.stderr)
        print('Tests failed in:', ', '.join(failed), file=sys.stderr)
        ret = 1
    else:
        if total_test_count == 1:
            print('1 test happy.', file=sys.stderr)
        else:
            print('All %d tests happy.' % total_test_count, file=sys.stderr)
        ret = 0

    if with_coverage:
        coverage.stop()
        modules = [mod for name, mod in sys.modules.items()
