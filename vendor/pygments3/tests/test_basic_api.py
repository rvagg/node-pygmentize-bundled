...
def test_lexer_import_all():
    ...
    for x in list(lexers.LEXERS.keys()):
        c = getattr(lexers, x)()
...
def test_get_lexers():
    ...
    for cls, (_, lname, aliases, _, mimetypes) in list(lexers.LEXERS.items()):
...
def test_formatter_public_api():
    ...
    for formatter, info in list(formatters.FORMATTERS.items()):
...
class FiltersTest(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        for x in list(filters.FILTERS.keys()):
