...
def generate_lexer_docs():
    ...
    modules = {}
    moduledocstrings = {}
    for classname, data in sorted(iter(list(LEXERS.items())), key=lambda x: x[0]):
        module = data[0]
    ...
    for module, lexers in sorted(iter(list(modules.items())), key=lambda x: x[0]):
...
def generate_formatter_docs():
    ...
    out = []
    for cls, data in sorted(iter(list(FORMATTERS.items())),
